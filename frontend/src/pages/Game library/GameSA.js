...
const Game = () => {
  const [newGame, setNewGame] = useState({ name: '', image_path: '', availability: '', platform: '' , hourly_rate:'', game_rating:''});
  const [games, setGames] = useState([]);
  const [editGameIds, setEditGameIds] = useState({});

  // Function to fetch events and comments
  const fetchGames = async () => {
    try {
      const eventsResponse = await axios.get('http://localhost:5000/api/games');
